...
public class BaseActor extends Group {
    ...
    public Animation<TextureRegion> loadAnimationFromSheet(String fileName, int rows, int cols, float frameDuration, boolean loop) {
        Texture texture = new Texture(Gdx.files.internal("maze/" + fileName), true);
        texture.setFilter(TextureFilter.Linear, TextureFilter.Linear);
    ...
    public void setBoundaryPolygon(int numSides, float width, float height) {
        ...
        for (int i = 0; i < numSides; i++) {
            ...
            // x-coordinate
            vertices[2 * i] = w * MathUtils.cos(angle) + w;
            // y-coordinate
            vertices[2 * i + 1] = h * MathUtils.sin(angle) + h;
        }
        boundaryPolygon = new Polygon(vertices);
    }
