...
public class Room extends BaseActor {
    ...
    public Room(float x, float y, Stage stage, float roomWidth, float roomHeight) {
        super(x, y, stage);
        loadTexture("dirt.png");
        setWidth(roomWidth);
        setHeight(roomHeight);

        // Calculate thickness based on room dimensions
        float thickness = Math.min(roomWidth, roomHeight) * 0.09f; // Adjust scaling factor as needed

        wallArray[SOUTH] = new Wall(x, y, roomWidth, thickness, stage);
        wallArray[WEST] = new Wall(x, y, thickness, roomHeight, stage);
        wallArray[NORTH] = new Wall(x, y + roomHeight - thickness, roomWidth, thickness, stage);
        wallArray[EAST] = new Wall(x + roomWidth - thickness, y, thickness, roomHeight, stage);
    }
